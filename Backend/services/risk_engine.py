...
RULES = [
    ("has_ip_address",                 0.20, "URL uses an IP address instead of a domain name"),
    ("has_at_symbol",                  0.15, "URL contains @ symbol — possible redirect trick"),
    ("brand_in_subdomain",             0.20, "URL subdomain impersonates a known brand"),
    ("has_punycode",                   0.15, "Domain uses internationalized characters (possible homograph)"),
    ("password_form_submits_external", 0.20, "Login form submits credentials to a different domain"),
    ("suspicious_tld",                 0.10, "Domain uses a TLD commonly associated with phishing"),
    ("has_port",                       0.08, "URL contains a non-standard port number"),
    ("num_subdomains_excessive",       0.10, "URL has excessive subdomains"),
    ("hyphen_count_excessive",         0.08, "Domain contains many hyphens"),
    ("https_absent",                   0.05, "Site does not use HTTPS"),
    ("url_length_excessive",           0.05, "Unusually long URL"),
    ("uses_shortening_service",        0.10, "URL uses a link shortening service"),
    ("excessive_url_encoding",         0.08, "URL contains excessive percent-encoding"),
    ("has_password_field",             0.05, "Page contains a login form"),
    ("favicon_is_external",            0.10, "Page favicon loaded from external domain"),
    ("hidden_iframes",                 0.10, "Page contains hidden iframes"),
    ("right_click_disabled",           0.08, "Page disables right-click (prevents inspection)"),
    ("null_links_excessive",           0.08, "Many non-functional links on page"),
    ("thin_page_with_login",           0.10, "Thin page with login form (minimal content)"),
    ("has_meta_refresh",               0.08, "Page uses auto-redirect"),
    ("external_resource_ratio_high",   0.08, "Most page resources loaded from external domains"),
]
...
def evaluate_risk(features):
    ...
    for rule_key, weight, reason in RULES:
